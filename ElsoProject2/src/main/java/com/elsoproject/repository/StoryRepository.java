...
@Repository
public class StoryRepository {// ez a híd az adatbázis az adatbázis és a kód között itt a Storyk tudjanak
								// áramlani
	...
	public List<Story> findAll() {
		...
		return jdbc.query(sql, (rs, i) -> new Story(rs.getLong("id"), rs.getString("title"), rs.getString("content"),
				rs.getDate("posted"), rs.getLong("blogger_id")));

	}

	public Story findByTitle(String title) {
		String sql = "select * from story where title = ? order by posted";
		return jdbc.queryForObject(sql, new Object[] { title }, new RowMapper<Story>() {
